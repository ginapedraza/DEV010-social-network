    ...
    return __PRIVATE_unguardedEventListen(c, WebChannel.EventType.OPEN, () => {
      ...
      if (!h) {
        ...
        if (o2) {
          ...
          void 0 === n3 && (n3 = v.INTERNAL, i3 = "Unknown error status: " + t3 + " with message " + o2.message), h = true, P2.Po(new FirestoreError(n3, i3)), c.close();
        } else
...
function query(e, t, ...n) {
  ...
  t instanceof AppliableConstraint && r2.push(t), r2 = r2.concat(n), function __PRIVATE_validateQueryConstraintArray(e2) {
    ...
    if (t2 > 1 || t2 > 0 && n2 > 0)
      throw new FirestoreError(v.INVALID_ARGUMENT, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
  }(r2);
  for (const t2 of r2)
    e = t2._apply(e);
  return e;
    ...
    const t = __PRIVATE_newUserDataReader(e.firestore), n = function __PRIVATE_newQueryFilter(e2, t2, n2, r2, i, s, o) {
      ...
      if (i.isKeyField()) {
        if ("array-contains" === s || "array-contains-any" === s)
          throw new FirestoreError(v.INVALID_ARGUMENT, `Invalid Query. You can't perform '${s}' queries on documentId().`);
        if ("in" === s || "not-in" === s) {
...
class QueryOrderByConstraint extends QueryConstraint {
  ...
  _apply(e) {
    const t = function __PRIVATE_newQueryOrderBy(e2, t2, n) {
      if (null !== e2.startAt)
        throw new FirestoreError(v.INVALID_ARGUMENT, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
      if (null !== e2.endAt)
        throw new FirestoreError(v.INVALID_ARGUMENT, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
...
function __PRIVATE_parseDocumentIdValue(e, t, n) {
  if ("string" == typeof (n = getModularInstance(n))) {
    if ("" === n)
      throw new FirestoreError(v.INVALID_ARGUMENT, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
    if (!__PRIVATE_isCollectionGroupQuery(t) && -1 !== n.indexOf("/"))
      throw new FirestoreError(v.INVALID_ARGUMENT, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${n}' contains a '/' character.`);
    const r2 = t.path.child(ResourcePath.fromString(n));
    if (!DocumentKey.isDocumentKey(r2))
      throw new FirestoreError(v.INVALID_ARGUMENT, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${r2}' is not because it has an odd number of segments (${r2.length}).`);
...
function __PRIVATE_validateDisjunctiveFilterElements(e, t) {
  if (!Array.isArray(e) || 0 === e.length)
    throw new FirestoreError(v.INVALID_ARGUMENT, `Invalid Query. A non-empty array is required for '${t.toString()}' filters.`);
}
function __PRIVATE_validateNewFieldFilter(e, t) {
  if (t.isInequality()) {
    ...
    if (null !== n2 && !n2.isEqual(r2))
      throw new FirestoreError(v.INVALID_ARGUMENT, `Invalid query. All where filters with an inequality (<, <=, !=, not-in, >, or >=) must be on the same field. But you have inequality filters on '${n2.toString()}' and '${r2.toString()}'`);
...
function __PRIVATE_validateOrderByAndInequalityMatch(e, t, n) {
  if (!n.isEqual(t))
    throw new FirestoreError(v.INVALID_ARGUMENT, `Invalid query. You have a where filter with an inequality (<, <=, !=, not-in, >, or >=) on field '${t.toString()}' and so you must also use '${t.toString()}' as your first argument to orderBy(), but your first orderBy() is on field '${n.toString()}' instead.`);
}
...
!function __PRIVATE_registerFirestore(e, t = true) {
  ...
  }, "PUBLIC").setMultipleInstances(true)), registerVersion(S, "4.1.0", e), registerVersion(S, "4.1.0", "esm2017");
}();
...
const azul = "/DEV010-social-network/assets/azul.18914341.png";
const brazoLike = "/DEV010-social-network/assets/brazoLike.73d9a89b.png";
const brazoLiked = "/DEV010-social-network/assets/brazoLiked.f7d216d5.png";
const trash = "/DEV010-social-network/assets/trash.c80b58f6.png";
const editar = "/DEV010-social-network/assets/editar.77201620.png";
const loginWithGoogle = () => {
  const provider = new GoogleAuthProvider();
  return signInWithPopup(auth, provider).then((result) => result).catch((error2) => {
    throw error2;
  });
};
...
const showPostsProfile = async () => {
  ...
  postsSnapshot.forEach((doc2) => {
    ...
    if (isCurrentUserPost) {
      ...
      editButton.alt = "Editar publicaci\xF3n";
      const deleteButton = document.createElement("img");
      const buttonDelete = document.createElement("button");
      buttonDelete.classList.add("button-delete");
      deleteButton.classList.add("delete-button");
      deleteButton.src = trash;
      deleteButton.alt = "Eliminar publicaci\xF3n";
      ...
      if (editButton) {
        editButton.addEventListener("click", () => {
          ...
          editDescription.textContent = "Edita tu publicaci\xF3n";
          ...
          closeEdit.alt = "Cerrar pantalla de edici\xF3n";
          ...
          saveButton.addEventListener("click", async () => {
            ...
            await updateDoc(doc2.ref, { post: editedPost });
      ...
      if (deleteButton) {
        deleteButton.addEventListener("click", () => {
          ...
          editDescription.textContent = "\xBFEst\xE1s seguro que quieres eliminar tu publicaci\xF3n?";
          ...
          acceptButton.addEventListener("click", async () => {
            try {
              ...
              console.error("error deleting the post:", error2);
            } finally {
              popUpDelete.remove();
            }
          });
...
const showPosts = async () => {
  ...
  postsSnapshot.forEach((doc2) => {
    ...
    likeImage.alt = "Dar like a la publicaci\xF3n";
    ...
    likeButton.addEventListener("click", async (e) => {
      ...
      try {
        if (userLiked === false) {
          ...
          await updateDoc(doc2.ref, { likes: tempArrayLikes });
          await updateDoc(doc2.ref, { likesCount: arrayLikesLength });
        ...
        console.error("Error updating the post:", error2);
      }
    });
    const getReduceButton = document.querySelector("#reduceButton");
    const getIncreaseButton = document.querySelector("#increaseButton");
...
const logoHome = "/DEV010-social-network/assets/logoHome.63f4ba05.png";
function home(navigateTo2) {
  ...
  slogan.textContent = "\xA1Comparte tus \xE9xitos, inspira tu Fitness!";
  emailButton.textContent = "Correo electr\xF3nico";
  googleButton.textContent = "Google";
  titleLogin.textContent = "Inicia sesi\xF3n";
  titleRegister.textContent = "Si no tienes cuenta";
  registerButton.textContent = "Reg\xEDstrate";
...
const logoLogin = "/DEV010-social-network/assets/logoLogin.3f7b6d41.png";
const blackEye = "/DEV010-social-network/assets/blackEye.673fd9d4.png";
function login(navigateTo2) {
  ...
  resetPass.textContent = "\xBFOlvidaste tu contrase\xF1a?";
  restorePass.textContent = "Recup\xE9rala aqu\xED";
  title.textContent = "Inicia sesi\xF3n";
  buttonLogin.textContent = "Ingresar";
  slogan.textContent = "\xA1Juntos podemos lograr grandes resultados!";
  buttonReturn.textContent = "Volver atr\xE1s";
  let click = false;
  ...
  buttonLogin.addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (errorCode === "auth/wrong-password") {
        errorAlert.textContent = "Contrase\xF1a incorrecta. Verifica tus datos.";
      }
...
function register(navigateTo2) {
  ...
  const errorAlert = document.createElement("p");
  errorAlert.classList.add("error");
  titleRegister.textContent = "Reg\xEDstrate";
  inputEmail.placeholder = "Correo electr\xF3nico";
  inputUser.placeholder = "Nombre de usuario";
  inputPass.placeholder = "Contrase\xF1a";
  inputConfirmPass.placeholder = "Repetir contrase\xF1a";
  buttonReturn.textContent = "Volver atr\xE1s";
  slogan.textContent = "\xA1Bienvenido a la aventura fit!";
  ...
  sendEmailButton.addEventListener("click", async (e) => {
    ...
    const name2 = inputUser.value;
    const password = inputPass.value;
    const confirmPass = inputConfirmPass.value;
    if (password !== confirmPass) {
      registerForm.reset();
      errorAlert.textContent = "Las contrase\xF1as no coinciden. Intenta nuevamente";
    }
    if (password === confirmPass) {
      try {
        const result = await createUser(email, password);
        await sendEmailVerification(auth.currentUser);
        await updateProfile(result.user, { displayName: name2 }).then(() => {
          navigateTo2("/mailVerification");
        });
      } catch (error2) {
        const errorCode = error2.code;
        ...
        if (inputPass.value.length < 6) {
          errorAlert.textContent = "La contrase\xF1a debe tener al menos 6 caracteres.";
          return errorAlert;
        }
...
const ProfileButton = "/DEV010-social-network/assets/ProfileButton.0e004445.png";
const reducir = "/DEV010-social-network/assets/reducir.c62d2326.png";
const aumentar = "/DEV010-social-network/assets/aumentar.408c47a9.png";
const letraNormal = "/DEV010-social-network/assets/letraNormal.b3f03c07.png";
const logOutImg = "/DEV010-social-network/assets/logOutImg.63b05d86.png";
function feed(navigateTo2) {
  ...
  reduceImg.alt = "Disminuir tama\xF1o de la fuente";
  ...
  increaseImg.alt = "Aumentar tama\xF1o de la fuente";
  ...
  normalImg.alt = "Tama\xF1o normal de la fuente";
  ...
  buttonLogout.textContent = "Cerrar sesi\xF3n";
  const imgLogout = document.createElement("img");
  imgLogout.src = logOutImg;
  imgLogout.alt = "Cerrar sesi\xF3n";
  ...
  textArea.placeholder = "\xBFQu\xE9 quieres compartir hoy?";
...
const atencion = "/DEV010-social-network/assets/atencion.8a80a80e.png";
function noFeed(navigateTo2) {
  ...
  messageToHome.textContent = "A\xFAn no has iniciado sesi\xF3n, por favor vuelve al inicio para que puedas acceder al contenido de la web.";
...
function resetPassword(navigateTo2) {
  ...
  const buttonReturn = document.createElement("button");
  buttonReturn.classList.add("button-return");
  inputResetPass.placeholder = "Ingresa tu correo electr\xF3nico";
  const errorAlert = document.createElement("p");
  errorAlert.classList.add("error");
  title.textContent = "\xBFOlvidaste tu contrase\xF1a?";
  ...
  buttonResetPass.addEventListener("click", async (e) => {
    e.preventDefault();
    sendPasswordResetEmail(auth, inputResetPass.value).then(() => {
      errorAlert.textContent = "Hemos enviado un link a tu correo para recuperar tu contrase\xF1a";
      ...
      const errorCode = error2.code;
      if (errorCode === "auth/user-not-found") {
        errorAlert.textContent = "Correo electr\xF3nico no encontrado";
      }
      if (errorCode !== "auth/user-not-found") {
        errorAlert.textContent = "Hubo un error al recuperar tu contrase\xF1a";
      }
    });
  });
...
function mailVerificacion(navigateTo2) {
  ...
  buttonLogin.textContent = "Ya verifiqu\xE9 mi correo";
  const messageSection = document.createElement("section");
  messageSection.classList.add("message-section");
  const messageToHome = document.createElement("h5");
  messageToHome.classList.add("message-tohome");
  messageToHome.textContent = "Hemos enviado el link de verificaci\xF3n a tu correo. Revisa la bandeja de entrada y verifica tu email";
...
const ButtonHome = "/DEV010-social-network/assets/ButtonHome.e6407be4.png";
